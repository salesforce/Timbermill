...
public class TimberLogLocalTest extends TimberLogTest {
    ...
    public static void init()  {
        ...
        LocalOutputPipe.Builder pipeBuilder = new LocalOutputPipe.Builder().numberOfShards(1).numberOfReplicas(0).url(elasticUrl).deletionCronExp(null).persistentFetchCronExp("0/5 * * 1/1 * ? *") // fetch every 2 seconds
        ...
        if (diskHandler==null){
            diskHandler = new SQLJetDiskHandler(3,3,"/tmp");
        }
        ...
        client = new ElasticsearchClient(elasticUrl, 1000, 1, null, null, null,
                7, 100, 1000000000,3, 3,3, false,null);
    ...
    public static LocalOutputPipe buildLocalOutputPipeForTest(LocalOutputPipe.Builder pipeBuilder,SQLJetDiskHandler diskHandler) {
