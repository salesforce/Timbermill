...
public class TasksMergerJobsTest {
	...
	public static void init() {
		...
		client.setOldIndex(previousIndex);
		client.setCurrentIndex(currentIndex);
	...
	public void testStartSuccessDifferentIndex() throws InterruptedException {
		...
		client.index(oldTasks, client.getOldIndex());
		TimberLogTest.waitForEvents(id, Task.TaskStatus.UNTERMINATED);
		client.index(newTasks, client.getCurrentIndex());
		TimberLogTest.waitForEvents(id, 2);
		tasksMergerJobs.execute(context);

		TimberLogTest.waitForEvents(id, Task.TaskStatus.SUCCESS);
		Task task = client.getTaskById(id);

		assertNotNull(task);
		assertEquals(Task.TaskStatus.SUCCESS, task.getStatus());
	...
	public void testSuccessStartDifferentIndex() throws InterruptedException {
		...
		client.index(oldTasks, client.getOldIndex());
		TimberLogTest.waitForEvents(id, Task.TaskStatus.PARTIAL_SUCCESS);
		client.index(newTasks, client.getCurrentIndex());
		TimberLogTest.waitForEvents(id, 2);
		tasksMergerJobs.execute(context);
	...
	public void testStartErrorDifferentIndex() throws InterruptedException {
		...
		TimberLogTest.waitForEvents(id, Task.TaskStatus.ERROR);
		Task task = client.getTaskById(id);

		assertNotNull(task);
		assertEquals(Task.TaskStatus.ERROR, task.getStatus());
		assertEquals(CTX_1, task.getCtx().get(CTX_1));
		assertEquals(CTX_2, task.getCtx().get(CTX_2));
		assertEquals(1, task.getMetric().get(METRIC_1).intValue());
		assertEquals(2, task.getMetric().get(METRIC_2).intValue());
	...
	public void testErrorStartDifferentIndex() throws InterruptedException {
		...
		client.index(oldTasks, client.getOldIndex());
		TimberLogTest.waitForEvents(id, Task.TaskStatus.PARTIAL_ERROR);
		client.index(newTasks, client.getCurrentIndex());
		TimberLogTest.waitForEvents(id, 2);
		tasksMergerJobs.execute(context);
